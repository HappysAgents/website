...
## PARA Knowledge Graph (Extension to Memory System)

The `memory/` folder uses PARA structure for the knowledge graph layer:

- `memory/projects/` — active work with a goal/deadline. Move to archives when done.
- `memory/areas/` — ongoing responsibilities with no end date (people, roles, companies)
- `memory/resources/` — reference material you might need later
- `memory/archives/` — completed or inactive items. Never delete — archive instead.

Each entity (person, project, company) gets its own subfolder containing:
- `summary.md` — short overview, load this first for quick context
- `facts.md` — atomic facts, only load when the summary isn't enough

**Fact rules:**
- Facts are never deleted. When something changes, mark the old fact superseded and add a new one.
- Use relatedEntities to cross-reference other entities — this is what makes it a graph.

## Tacit Knowledge File

`tacit-knowledge.md` (in workspace root) contains R's working patterns, communication preferences, and hard rules. Read this when uncertain how R wants something handled. Update it only when a new stable pattern is confirmed across multiple sessions — not after a single interaction.

## Memory Write Routing (Quick Reference)

| What | Where |
|------|-------|
| Durable facts about projects/people | PARA entity facts.md |
| Infrastructure, decisions, rules | MEMORY.md |
| What happened this session | memory/YYYY-MM-DD.md |
| R's stable patterns and preferences | tacit-knowledge.md |
